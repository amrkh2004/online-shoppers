...
class ShopperInputSchema(BaseModel):
    ...
    Administrative_Duration: float = Field(
        0.0, ge=0.0, description="Total time spent on administrative pages"
    )
    Informational: int = Field(0, ge=0, description="Number of informational pages visited")
    Informational_Duration: float = Field(
        0.0, ge=0.0, description="Total time spent on informational pages"
    )
    ProductRelated: int = Field(1, ge=0, description="Number of product-related pages visited")
    ProductRelated_Duration: float = Field(
        10.0, ge=0.0, description="Total time spent on product-related pages"
    )
    BounceRates: float = Field(
        0.0, ge=0.0, le=1.0, description="Average bounce rate of pages visited"
    )
